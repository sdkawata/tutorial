...
Uploader.makeKey = function (filename) {
    now=new Date();
  return this.config.prefix+(""+now.getFullYear()+"_"+now.getMonth()+"_"+now.getDay()+"_"+now.getHours()+ "_" + now.getMinutes() + "_" + now.getSeconds() + "_" +now.getMilliseconds() + "_" +  filename );
};
 ...
 */
Uploader.uploadToS3 = function(file, url, acl, metadata, key)
{
  ...
  xhr.onload = function() {
    if(xhr.status == 200){
      Uploader.onUploadSuccess(file, key);
    } else {
...
Uploader.onUploadSuccess = function(file, key) {
